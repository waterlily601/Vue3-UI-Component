<template lang="">
  <div class="el-collapse">
    ...
    <slot></slot>
    <!-- <slot></slot>：这是一个插槽，用来接收外部传递的 CollapseItem 组件。
    CollapseItem 组件将作为子项展示在折叠面板中，用户可以传递多个 CollapseItem 组件。 -->
  </div>
</template>
...
// const props = defineProps<CollapseProps>()：使用 defineProps 定义组件接收的属性，
// 并指定它们的类型为 CollapseProps，包括 modelValue 和 accordion。
const props = defineProps<CollapseProps>()

// const emits = defineEmits<CollapseEmits>()：使用 defineEmits 定义组件将要触发的事件类型，
// 类型为 CollapseEmits，包含如 update:modelValue 和 change 事件。
const emits = defineEmits<CollapseEmits>()

// 存储当前展开的项的名称数组
// 用来存储当前展开的折叠项的名称。当有项展开或收起时，会更新这个数组。
const activeNames = ref<CollapseActiveName[]>([])
// 监听 props.modelValue 的变化，并更新 activeNames
// 这意味着外部可以通过 v-model 控制展开的折叠项。
